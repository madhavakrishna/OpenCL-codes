...
typedef struct { float x, y; int group; } point_t, *point;
...
point gen_xy(int count, float radius)
{
	float ang, r;
...
inline __attribute__((always_inline)) float dist2(point a, point b)
{
	float x = a->x - b->x, y = a->y - b->y;
	return x*x + y*y;
}

inline __attribute__((always_inline)) int nearest(point pt, point cent, int n_cluster, float *d2)
{
	int i, min_i;
	point c;
	float d, min_d;
...
#	define for_len for (j = 0, p = pts; j < len; j++, p++)
	int j;
	int n_cluster;
	float sum, *d = malloc(sizeof(float) * len);
...
#	define H 400
	...
	float *colors = malloc(sizeof(float) * n_cluster * 3);
